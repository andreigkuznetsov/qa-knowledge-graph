...
class ScenarioIdentityGroupComposerV1Test {
    ...
    static Fixture fixture(){var ra=RawSourceMemberFingerprint.calculate(new byte[]{1});var rb=RawSourceMemberFingerprint.calculate(new byte[]{2});var members=List.of(new RepositoryCaptureFingerprintInput.CapturedMember("a.json",1,ra),new RepositoryCaptureFingerprintInput.CapturedMember("b.json",1,rb));var in=new RepositoryCaptureFingerprintInput("repo","source-v1","profile","discovery-v1","path-v1","order-v1",RawSourceMemberFingerprint.ALGORITHM_IDENTIFIER,members,List.of());var fp=RepositoryCaptureFingerprintEncoder.fingerprint(in);var refs=List.of(new AttributedMemberOutcomeFingerprintInput.ParentCapturedMemberReference("repo","snap",fp,"a.json",1,ra),new AttributedMemberOutcomeFingerprintInput.ParentCapturedMemberReference("repo","snap",fp,"b.json",1,rb));return new Fixture(fp,refs,RepositoryCaptureAttestation.verified("repo","snap",in,fp,refs,List.of()));}
