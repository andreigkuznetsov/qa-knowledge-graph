...
class ScenarioIdentityGroupComposerV1Test {
    ...
    static VerifiedScenarioIdentityGroupV1 compose(ScenarioOccurrenceCompositionOutcomeV1...o){return ScenarioIdentityGroupComposerV1.compose(ScenarioIdentityGroupComposerV1.DUPLICATE_OUTCOME_CONTRACT,ID,List.of(o));}
    static ScenarioOccurrenceCompositionOutcomeV1 attempt(NormalizedScenarioOccurrenceInputV1 o){return ScenarioOccurrenceCompositionAttemptV1.attemptScenarioOccurrenceCompositionV1(o);}
    static NormalizedScenarioOccurrenceInputV1 input(Fixture f,String path,int index,String title){return input(f,path,index,title,ScenarioSemanticFingerprintEncoder.SOURCE_NORMALIZATION_VERSION);}
    static NormalizedScenarioOccurrenceInputV1 input(Fixture f,String path,int index,String title,String sourceVersion){var p=f.refs.stream().filter(x->x.normalizedRepositoryRelativePath().equals(path)).findFirst().orElseThrow();
    ...
    static Fixture fixture(){return fixture("a.json","b.json");}
    static Fixture fixture(String... paths){var sorted=Arrays.stream(paths).sorted(ScenarioIdentityGroupComposerV1Test::codePoints).toList();var members=new ArrayList<RepositoryCaptureFingerprintInput.CapturedMember>();var refsRaw=new ArrayList<RawSourceMemberFingerprint>();for(int i=0;i<sorted.size();i++){var raw=RawSourceMemberFingerprint.calculate(new byte[]{(byte)(i+1)});refsRaw.add(raw);members.add(new RepositoryCaptureFingerprintInput.CapturedMember(sorted.get(i),1,raw));}var in=new RepositoryCaptureFingerprintInput("repo","source-v1","profile","discovery-v1","path-v1","order-v1",RawSourceMemberFingerprint.ALGORITHM_IDENTIFIER,members,List.of());var fp=RepositoryCaptureFingerprintEncoder.fingerprint(in);var refs=new ArrayList<AttributedMemberOutcomeFingerprintInput.ParentCapturedMemberReference>();for(int i=0;i<sorted.size();i++)refs.add(new AttributedMemberOutcomeFingerprintInput.ParentCapturedMemberReference("repo","snap",fp,sorted.get(i),1,refsRaw.get(i)));return new Fixture(fp,List.copyOf(refs),RepositoryCaptureAttestation.verified("repo","snap",in,fp,refs,List.of()));}
    ...
    record Fixture(RepositoryCaptureFingerprint fp,List<AttributedMemberOutcomeFingerprintInput.ParentCapturedMemberReference> refs,RepositoryCaptureAttestation att){}
}
